...
class TestConstructor:
    def test_move_to_section_bread_by_click_not_authorised_user_open_section_bread(self, driver):
    ...
    def test_move_to_section_sauce_by_click_not_authorised_user_open_section_sauce(self, driver):
        driver.get(URL.HOME_PAGE_URL)
        WebDriverWait(driver,3).until(expected_conditions.element_to_be_clickable(ConstructorLocators.SAUCE_SECTION))
        driver.find_element(*ConstructorLocators.SAUCE_SECTION).click()
        assert driver.find_element(*ConstructorLocators.ACTIVE_SECTION).text == 'Соусы'

    def test_move_to_section_filling_by_click_not_authorised_user_open_section_filling(self, driver):
        driver.get(URL.HOME_PAGE_URL)
        WebDriverWait(driver,3).until(expected_conditions.element_to_be_clickable(ConstructorLocators.SAUCE_SECTION))
